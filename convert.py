...
if __name__ == "__main__":
    ...
    unet_state_dict = convert_unet_state_dict({k[5:]: v for k, v in state_dict.items() if k.startswith("unet")})
    unet_state_dict = {"model.diffusion_model." + k: v for k, v in unet_state_dict.items()}

    # Convert the VAE model
    vae_state_dict = convert_vae_state_dict({k[4:]: v for k, v in state_dict.items() if k.startswith("vae")})
