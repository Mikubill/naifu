...
def create_diffusers_schedular(original_config):
    from transformers import DDIMScheduler
...
def create_ldm_bert_config(original_config):
    from transformers import LDMBertConfig
...
def convert_ldm_unet_checkpoint(checkpoint, config, path=None, extract_ema=False):
    ...
    if sum(k.startswith("model_ema") for k in keys) > 100:
        ...
        if extract_ema:
            ...
            print("In this conversion only the non-EMA weights are extracted. ")
...
def convert_ldm_bert_checkpoint(checkpoint, config):
    from transformers import LDMBertModel
...
def convert_ldm_openclip_checkpoint(checkpoint):
...
def convert_ldm_clip_checkpoint(checkpoint):
    ...
    for key in keys:
        if key.startswith("cond_stage_model.transformer"):
            stripped_key =  key[len("cond_stage_model.transformer.") :]
            if not stripped_key.startswith("text_model"):
                stripped_key = "text_model." + stripped_key
            text_model_dict[stripped_key] = checkpoint[key]
