...
class MT5Embedder(nn.Module):
    def __init__(
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_dir, **model_kwargs)
    ...
    def get_text_embeddings(self, texts, attention_mask=True, layer_index=-1):
        ...
        outputs = self.model(
            ...
            attention_mask=text_tokens_and_mask["attention_mask"].to(self.device.device),
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs_cis_img=None):
        ...
        scores = F.scaled_dot_product_attention(q, k, v)
        context = scores.transpose(1, 2)
...
class DiTModel(nn.Module):
    """
    DiT: Diffusion model with a Transformer backbone.
