...
def attention_pytorch(qkv, num_heads):
    q, k, v, heads = qkv[0], qkv[1], qkv[2], num_heads
...
class SelfAttentionContext(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dtype=None, device=None):
        super().__init__()
        dim_head = dim // heads
        inner_dim = dim
        ...
        self.qkv = nn.Linear(dim, dim * 3, bias=True, dtype=dtype, device=device)

        self.proj = nn.Linear(inner_dim, dim, dtype=dtype, device=device)

    def forward(self, x):
        qkv = self.qkv(x)
        q, k, v = split_qkv(qkv, self.dim_head)
        x = attention_pytorch((q.reshape(q.shape[0], q.shape[1], -1), k, v), self.heads)
        return self.proj(x)

class ContextProcessorBlock(nn.Module):
    def __init__(self, context_size, dtype=None, device=None):
        super().__init__()
        self.norm1 = nn.LayerNorm(context_size, elementwise_affine=False, eps=1e-6, dtype=dtype, device=device)
        self.attn = SelfAttentionContext(context_size, dtype=dtype, device=device)
        self.norm2 = nn.LayerNorm(context_size, elementwise_affine=False, eps=1e-6, dtype=dtype, device=device)
        self.mlp = Mlp(in_features=context_size, hidden_features=(context_size * 4), act_layer=lambda: nn.GELU(approximate="tanh"), drop=0, dtype=dtype, device=device)
...
class ContextProcessor(nn.Module):
    def __init__(self, context_size, num_layers, dtype=None, device=None):
        super().__init__()
        self.layers = torch.nn.ModuleList([ContextProcessorBlock(context_size, dtype=dtype, device=device) for i in range(num_layers)])
        self.norm = nn.LayerNorm(context_size, elementwise_affine=False, eps=1e-6, dtype=dtype, device=device)
...
class MMDiT(nn.Module):
    ...
    def __init__(
        ...
        if adm_in_channels is not None:
            ...
            self.y_embedder = VectorEmbedder(adm_in_channels, self.hidden_size, dtype=dtype, device=device)

        if context_processor_layers is not None:
            self.context_processor = ContextProcessor(context_size, context_processor_layers, dtype=dtype, device=device)
        else:
        ...
        self.final_layer = FinalLayer(self.hidden_size, patch_size, self.out_channels, dtype=dtype, device=device)
