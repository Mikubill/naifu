...
class ResBlock(nn.Module):
    ...
    def _forward(self, x, x_skip=None):
        x_res = x
    ...
    def forward(self, x, x_skip=None):
        if self.training and self.gradient_checkpointing:
            ...
            x = torch.utils.checkpoint.checkpoint(create_custom_forward(self._forward), x, x_skip)
        else:
            x = self._forward(x)
        ...
        if self.training and self.gradient_checkpointing:
            def create_custom_forward(func):
                def custom_forward(*inputs):
                    return func(*inputs)
                return custom_forward
            x = torch.utils.checkpoint.checkpoint(create_custom_forward(self._forward), x, kv)
        else:
            x = self._forward(x)
        ...
        if self.training and self.gradient_checkpointing:
            def create_custom_forward(func):
                def custom_forward(*inputs):
                    return func(*inputs)
                return custom_forward
            x = torch.utils.checkpoint.checkpoint(create_custom_forward(self._forward), x)
        else:
            x = self._forward(x)
